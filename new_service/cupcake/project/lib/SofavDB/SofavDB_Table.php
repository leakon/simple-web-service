 ...
 *
 * @notice	Return value when calling save(), do NOT leave jus a "return" statment if nothing was modified.
 */
abstract class SofavDB_Table {
	...
	public function __set($key, $value) {
		...
		//	$this->arrProperties[$key]	= $value;
			...
			throw new Exception(sprintf("[SofavDB_Table] The property [%s] you set is NOT defined. Defined properties:[%s]",
						$key, implode(', ', array_keys($this->arrProperties))));
		}
		return	$value;
	}
	...
	public function __get($key) {
		if (array_key_exists($key, $this->arrProperties)) {
			...
			throw new Exception(sprintf("[SofavDB_Table] The property [%s] you get is NOT defined. Defined properties:[%s]",
						$key, implode(', ', array_keys($this->arrProperties))));
			return	NULL;
		}
	}
