...
class Action_Security {

	public static function makeSignedString($inputString) {
		return	md5(CydSecret::SECRET_KEY . $inputString);
	}
	...
	*/
	 ...
	 * 检查用户邮件是否在管理员列表之内
	 *
	 * @param	mixed	$mixedAdminUser		sfUser 对象 或者 mail 字符串
	 * @return	bool	是否是管理员
	 */
	public static function isAdmin($mixedAdminUser) {

		if (is_string($mixedAdminUser)) {
			$mail	= $mixedAdminUser;
		} else {
			$mail	= $mixedAdminUser->get('mail');
		}

		return	Sofav_Data_User::isAdmin($mail);
	}
